...
def adv_parse(line):
    ...
    if token == 'give':
        obj = ' '.join(tokens[:2])
        return (token, obj, tokens[-1])
    elif token == 'ask':
        """ YOUR CODE HERE """
        what = ' '.join(tokens[2:])
        return (token, tokens[0], what)
...
def adv_eval(exp):
    ...
    if is_person(exp):
        ...
        """ YOUR CODE HERE """
        return give(adv_eval(exp[-1]), (exp[1]))
    elif exp[0] == 'ask':
        """ YOUR CODE HERE """
        return ask(adv_eval(exp[1]), (exp[-1]))
    else:
        """ YOUR CODE HERE """
        return adv_apply(adv_eval(exp[0]), (exp[1]))
