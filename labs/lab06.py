...
def inhexing(lst, hex, n):
    ...
    for i in range(len(lst)):
        if i % n == 0: #if it's the nth element, return the hex, then add to list 
            result += [hex(i)]
        else:
            result += [i] #return the element and add it to the list
    return result
def deep_inhexing(lst, hex, n):
    ...
    def helper(lst, counter):
        if lst == []: #if an element in the list is itself a list, evaluate that list first 
            return []
        first, rest = lst[0], lst[1:]
        if type(first) == list: 
            return [helper(first, 1)] + helper(rest, counter+1)
        elif counter % n == 0: #if the first of the list == the nth element
            return [hex(first)] + helper(rest, counter+1)
        else:
            return [first] + helper(rest, counter+1) #if the first of the list != the nth element, just return the element
    return helper(lst, 1)
...
def dejavu(t, n):
    ...
    if children(t) == []:
        return datum(t) == n
    for kids in children(t):
        if dejavu(kids, n - datum(t)):
            return True
    return False
...
def phi_approx():
    ...
    True
    """
    return easy_find_zero(lambda x: pow(x, 2) - x - 1)
