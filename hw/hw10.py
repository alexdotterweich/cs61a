...
class HuffmanTree(Tree):
    ...
    def merge_two_smallest(self):
        ...
        second_small = min(self.children, key=lambda x: x.frequency)
        self.children.remove(second_small)
        new_tree = HuffmanTree(smallest, second_small)
    ...
    def encode_character(self, character):
        ...
        if character in self.right_child.datum: 
    ...
    def decode_character(self, code):
        ...
        "*** YOUR CODE HERE ***"
        if code[0] == '0':
            return self.left_child.decode_character(code[1:])
        if code[0] == '1':
            return self.right_child.decode_character(code[1:])

        #returns the decoded character and the undecoded bits
        #decodes one value at a time
