...
def do_cons_stream_form(vals, env):
    ...
    stream"""
    check_form(vals, 2, 2)
    return Stream(scheme_eval(vals.first, env), vals.second, env), None
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            if self.use_env:
                arg_list.append(env)
            value, environment = self.fn(*arg_list), None
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_function = do_lambda_form(Pair (target.second, vals.second), env)
        env.define(target.first, lambda_function[0])
...
def do_and_form(vals, env):
    ...
    if len(vals) == 1:
        return scheme_eval(vals[0], env), None
    for i in range(len(vals) - 1):
        if scheme_eval(vals[i], env) == scheme_false:
...
def do_cond_form(vals, env):
    ...
    return okay, None
# if there are multiple clauses that are true, want to return the first one.
# when a true clause is found, want to return second element in clause if 
# it has one, scheme_true otherwise
