    ...
    (cond ((null? list1) list2)
         ((null? list2) list1)
            ((comp (car list1) (car list2))
                   (append (list (car list1)) (merge comp list2 (cdr list1))))
...
(define (patterns)
  ...
  (define helper
    (cons-stream '()
      (interleave
        (stream-map over helper)
        (stream-map under helper))))
  (define (has-over-under lst)
    (and (member? 'over lst) (member? 'under lst)))
  (stream-filter has-over-under helper))
...
(define (tree-sums tree)
  (if (null? (children tree)) (list (entry tree))
     (map (lambda (x) (+ x (entry tree))) (flatten (map tree-sums (children tree))))))

; you want to add the entry to each of the children's values
