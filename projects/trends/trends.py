...
def get_word_sentiment(word):
    ...
    >>> sentiment_zvalue(get_word_sentiment('good'))
    0.875
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in word_list:
        ...
        if has_sentiment(sentiment): 
            sentiment_list.append(sentiment_value(sentiment))
    if sentiment_list == []:
        return make_sentiment(None)
    else: 
        return make_sentiment(sum(sentiment_list) / len(sentiment_list))
...
def find_state_center(polygons):
    ...
    for shape in polygons:
        cx += (find_centroid(shape)[0]) * (find_centroid(shape)[2])
        cy += (find_centroid(shape)[1]) * (find_centroid(shape)[2])
        area += find_centroid(shape)[2]    
